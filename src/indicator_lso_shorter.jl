...
function training!(model)
    ...
    train!(model,loss_function = "conditional likelihood", optim_options = (maxiter = 2,step_size=0.05))
    # finish with a smaller step size 
    train!(model,loss_function = "conditional likelihood", optim_options = (maxiter = 2,step_size=0.01))
end


path = "/Users/johnbuckner/github/UDEsWithSpatialReplicates/results/indicator/leave_site_out"
leave_site_out_indicator(model, training!, path, 6)
